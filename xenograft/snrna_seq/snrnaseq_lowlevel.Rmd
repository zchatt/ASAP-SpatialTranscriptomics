...
# Note cellranger_1y_hm did not have enough cells to integrate
seurat_list_human_sub <- seurat_list_human[c(1,4)]
features <- SelectIntegrationFeatures(object.list = seurat_list_human_sub, nfeatures = 3000)
...
FeaturePlot(merge.combined.sct, 
            reduction = "umap", 
            features = metrics,
            pt.size = 0.4, 
            sort.cell = TRUE)
...
markers_write <- markers[markers$p_val_adj < 0.05,]
row.names(markers_write) <- gsub("GRCh38--","",row.names(markers_write))
# write to file
write.table(markers_write, file="DEG_young.xenograft_cl0(unk)-cl1(DaN).txt", quote = FALSE, sep="\t")
