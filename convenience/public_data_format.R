...
########################################################################################################################################
### 5. Create a merged seurat object with Siletti (Carmen processed DA and Sandy processed other) and Webber snRNAseq datasets
########################################################################################################################################
...
### 5. Create a merged seurat object with Siletti (Carmen processed DA and Sandy processed other) and Webber snRNAseq datasets
########################################################################################################################################
seurat_silett <- LoadSeuratRds("/Users/zacc/USyd/spatial_transcriptomics/data/public_datasets/SNc.RN.ref.integ.harmony.rds")
load("/Users/zacc/USyd/spatial_transcriptomics/data/public_datasets/webber_re_seurat.Rdata")
load("/Users/zacc/USyd/spatial_transcriptomics/data/public_datasets/siletti_re_seurat.Rdata")


# remove sample id prefix in Sandy Siletti and make syntantically valid
new_names <- make.names(gsub("snc.rn _|snc_","",colnames(seurat_silett)))

# check Siletti_re names to Sandy's Siletti names
not_found <- colnames(seurat_silett_re)[!colnames(seurat_silett_re) %in% new_names]
not_found <- not_found[-grep("pPD",not_found)]

table(seurat_silett_re@meta.data[not_found,]$Tissue)

# there are 211 cells that cant be found within the "SNc.RN.ref.integ.harmony.rds" reference. 


not_found <- colnames(seurat_silett)[grep("GCCAGCACACCCTAAA",colnames(seurat_silett))]


table(seurat_silett_re@meta.data$Tissue["X10X193_3.GATGAGGGTGGAAATT"])


# convert Siletti_re names to Sandy's Siletti names
strings <- colnames(seurat_silett_re)
strings_modified <- ifelse(substr(strings, 1, 1) == "X", 
                           sub("^X", "snc_", strings), 
                           strings)
gsub()

table(strings_modified %in% make.names(colnames(seurat_silett)))

table(strings_modified %in% make.names(colnames(seurat_silett)))
strings_modified[!strings_modified %in% make.names(colnames(seurat_silett))]


colnames(seurat_silett)[grep("TTACGCCCAACAGCTT",colnames(seurat_silett))]

seurat_silett_re@meta.data$SampleID[!seurat_silett_re$SampleID %in% seurat_silett$sample_id]


table(seurat_silett_re$SampleID %in% seurat_silett$sample_id)


X10X236_2.CAGCACGGTCAACCTA

snc_10X230_1:GGAGGTAGTATGGTAA

seurat_silett_re$SampleID
